...
;
(defun at-most-one-color (n c k &optional (i (+ c 1)))
   (cond
     ((> c k) ())
     ((> i k) (at-most-one-color n (+ c 1) k (+ c 2)))
     (t (cons (list (- (node2var n c k)) (- (node2var n i k)))
              (at-most-one-color n c k (+ i 1))))))
...
;
(defun generate-edge-clauses (e k &optional (c 1))
  (let ((nodeA (first e))
        (nodeB (second e)))
    (cond
      ((> c k) ())
      (t (cons (list (- (node2var nodeA c k)) (- (node2var nodeB c k)))
               (generate-edge-clauses e k (+ c 1)))))))
